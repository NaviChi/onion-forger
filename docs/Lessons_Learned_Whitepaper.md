...
- **LESSON-136-001:** Conflux (`prefer_conflux`) is NOT available in arti-client 0.40.0. Our existing Mirror Striping (Phase 129) achieves the same effect at the application level — 4 circuits across 4 mirrors with Thompson Sampling selection. When Conflux becomes public API, it can be layered ON TOP of mirror striping for 2× improvement (each of our 4 mirror circuits would itself be a bonded pair).
