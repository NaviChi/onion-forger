...
## 2026-03-13 (Phase 144: Parallel Download Stall Prevention)
- **LESSON-144-001 (CRITICAL — TIMEOUT HIERARCHY):** LESSON-140-001 ("every .await MUST have a timeout") was violated by `scaffold_download()` — the single most important `.await` in the entire download pipeline. The fix required not just adding `tokio::time::timeout()`, but also cleanup of `DownloadControl` in timeout/error paths, heartbeat emission, and timeout escalation. **RULE: After any major refactor, audit all `.await` calls against LESSON-140-001.**
- **LESSON-144-002 (HIGH — STALL DETECTOR LOCATION):** The stall detector at the top of the outer loop was useless because the download happened inside an inner `while !pending_entries.is_empty()` loop. The detector only fires when re-entering the outer loop — which never happens during a stuck batch. **RULE: Stall/watchdog checks must run concurrently with the operation they monitor, not sequentially before it.**
- **LESSON-144-003 (HIGH — DOWNLOAD CONTROL MUTEX):** `activate_download_control()` uses an `OnceLock<Mutex<Option>>` global. If `clear_download_control()` isn't called (timeout, error, panic), all subsequent chunks silently fail with "A download is already active". **RULE: Global mutex acquisition paired with manual release must use RAII or ensure cleanup in ALL exit paths.**
- **LESSON-144-004 (MEDIUM — ADAPTIVE TIMEOUT):** Fixed timeouts fail in both directions: too short → false positives on slow networks, too long → defeats the purpose. The solution is `(base + per_file) × escalation_multiplier` where multiplier grows on consecutive timeouts (1.5×, max 3.0×) and resets on success.
- **LESSON-144-005 (MEDIUM — PROBE SILENCE):** The serial probe loop (3430-3510) in `start_batch_download` emitted no progress for up to 8+ minutes while probing 100+ files. Users thought the app was frozen. **RULE: Any loop taking >10s must emit periodic progress.**
- **LESSON-144-006 (HIGH — PARALLEL PROBES):** Serial probe across Tor = N×5s = unbounded latency. JoinSet with Semaphore(4) provides 4× speedup while bounded concurrency prevents circuit overload. **RULE: Any serial Tor request loop over >10 items MUST be parallelized with bounded concurrency.**
- **LESSON-144-007 (HIGH — COORDINATED THROTTLE):** Independent per-worker 2s backoff on 503 = all workers hammer the server simultaneously after pause expires. A shared Semaphore(2) with 500ms hold time ensures coordinated cooldown — workers naturally queue up instead of stampeding. This is the aria2 `--retry-wait` pattern adapted to async Rust.

## 2026-03-13 (Phase 142-IMPL: R1+R2+R3+R4 Implementation)
- **LESSON-142-IMPL-001 (HIGH — BOUNDED CHANNEL MIGRATION):** Switching from `mpsc::unbounded_channel()` to `mpsc::channel(200)` requires updating ALL sender call sites from `.send()` to `.try_send()`. The bounded channel's `.send()` is async and will not compile in sync contexts. In `frontier.rs`, `qilin.rs` (VFS flush closure), and `lib.rs` — all three locations must be updated together. The `try_send` approach is semantically better anyway: if the channel is full, entries are silently dropped and the consumer will catch up (backpressure).
- **LESSON-142-IMPL-002 (HIGH — HEDGE PARTIAL ONLY):** The hedged retry (R1) must only fire when **some but not all** files in a chunk failed. If 100% of files fail, hedging the entire batch wastes circuits — the stall detector's NEWNYM recovery is the correct mechanism for total failures. The 0-byte file check heuristic works because `scaffold_download` creates 0-byte placeholders for files it cannot download.
- **LESSON-142-IMPL-003 (MEDIUM — HOST SORTING COST):** URL parsing (`reqwest::Url::parse`) in the sort comparator runs once per comparison, not once per entry. For 100 entries, this is ~700 comparisons × 2 parses = ~1400 URL parses. On modern CPUs this is <1ms total cost. If profiling shows this as hot, pre-compute host keys before sorting.
- **LESSON-142-IMPL-004 (MEDIUM — BATCH DURATION TRACKING):** Only batches >0.1s are recorded for adaptive stall threshold to avoid contamination from cache-hit batches. The 16-element ring buffer keeps memory bounded. Using `cloned().fold(0.0f64, f64::max)` instead of `Float::max()` to avoid pulling in a dependency.

## 2026-03-13 (Phase 142: Exhaustive Cross-Industry Improvement Analysis)
- **LESSON-142-001 (CRITICAL — BOTTLENECK IDENTIFICATION):** The #1 remaining bottleneck from the Phase 140B test (201K entries, 1.07-1.83 MB/s) is **probe-stage failures** — 1,055 throttles + connection drops in 25 minutes. The event-driven pipeline (Phase 141) eliminated VFS polling overhead, but the download consumer still processes probes serially on a single circuit per file. Google's "The Tail at Scale" hedged request pattern directly addresses this by racing probes on 2 circuits.
- **LESSON-142-002 (HIGH — ANTI-PATTERN REGISTRY):** 8 optimization ideas were explicitly rejected after literature research + codebase cross-reference: (1) more isolated_client() views share guard bandwidth (LESSON-140C-001), (2) Conflux doubles HS setup cost (LESSON-136-001), (3) >4 base TorClients causes OOM (LESSON-140-002), (4) custom relay picking solves wrong bottleneck, (5) multiple Arti daemons cause ENDPOINT_UNREACHABLE (Phase 54), (6) Connection: close on hot paths kills keep-alive (LESSON-099-002), (7) optimistic streams break .onion handshake (PR-OPTIMISTIC-132-001), (8) 120-worker HFT bursts cause oversubscription (Phase 2.6). **RULE: Before implementing any download optimization, cross-check this anti-pattern list.**
- **LESSON-142-003 (HIGH — HOST GROUPING):** The download consumer processes 100-entry chunks in FIFO order regardless of host. This means consecutive files may target different storage nodes, wasting HTTP keep-alive connections. aria2 and IDM implicitly group by host. Sorting by `url.host()` before batch processing is zero-cost and gives 10-20% connection reuse benefit.
- **LESSON-142-004 (MEDIUM — STALL THRESHOLD):** The fixed 90s stall threshold (Phase 141B) is optimal for degraded networks but too conservative for healthy ones. On circuits averaging 2s latency, a stall should fire in ~6s, not 90s. Adaptive threshold = `3 × median_circuit_latency` with floor 30s / ceiling 180s.
- **LESSON-142-005 (MEDIUM — UNBOUNDED CHANNEL RISK):** The `mpsc::unbounded_channel` for download feed (Phase 141) can buffer 200K+ entries in memory during explosive discovery. SpaceX/Starlink AQM research shows bounded queues with natural backpressure are safer. Replacing with `channel(200)` provides backpressure without starvation.
- **LESSON-142-006 (LOW — NASA DTN ANALOGY):** NASA's Delay-Tolerant Networking "store-and-forward" pattern maps to our VFS persistence — entries survive circuit death and can be replayed. The VFS final sweep already implements this. No additional persistent queue is needed unless crash recovery during download becomes a user-reported issue.
