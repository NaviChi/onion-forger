...
#[test]
#[ignore = "live network test against onion target; run manually"]
fn live_lockbit_pipeline_default_ui_settings() {
    ...
    rt.block_on(async {
        ...
        if let Err(err) = download_result {
            panic!("download pipeline failed: {err}");
        }
        ...
        assert!(all > 0, "expected non-zero batch total");
        assert_eq!(fail, 0, "expected zero failed files");
        assert!(done > 0, "expected completed files");
        assert!(disk_bytes > 0, "expected non-empty output on disk");
    });
}
