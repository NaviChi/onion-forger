use anyhow::Result;
use crawli_lib::adapters::alphalocker::AlphaLockerAdapter;
...
async fn main() -> Result<()> {
    ...
    let seed_url = "http://3v4zoso2ghne47usnhyoe4dsezmfqhfv5v5iuep4saic5nnfpc6phrad.onion/gazomet.pl%20&%20cgas.pl/Files/";
    ...
    let adapter = AlphaLockerAdapter::default();
    ...
    tokio::select! {
        res = crawl_future => {
            let res: Result<Vec<crawli_lib::adapters::FileEntry>, _> = res;
            if let Ok(entries) = res {
                println!("Crawl completed before 5 minutes! Adapter explicitly extracted {} entries.", entries.len());
                for (i, entry) in entries.iter().take(5).enumerate() {
                    println!("  [{}] {} (Folder: {})", i, entry.path, entry.entry_type == crawli_lib::adapters::EntryType::Folder);
                }
            } else {
                println!("Crawl failed with an error.");
            }
        }
