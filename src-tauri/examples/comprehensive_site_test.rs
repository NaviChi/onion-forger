...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (swarm_guard, ports) =
        crawli_lib::tor::bootstrap_tor_cluster(app.handle().clone(), 3, 3).await?;
    ...
    for (i, target) in targets.iter().enumerate() {
        ...
        let adapter: Box<dyn CrawlerAdapter> = match adapter_name {
            ...
            _ => {
                println!(
                    "⚠️ SKIPPED -> Adapter '{}' is not implemented yet",
                    adapter_name
                );
                continue;
            }
            ...
            || url_lower.ends_with(".rar")
        {
            ...
            println!(
                "✅ SUCCESS -> Identified and parsed 1 entries in {:?}",
                start.elapsed()
            );
            println!("--------------------------------------------------\n");
            continue;
        ...
        match tokio::time::timeout(
            ...
            adapter.crawl(url, frontier, app.handle().clone()),
        )
        .await
        {
            Ok(Ok(entries)) => {
                ...
                println!(
                    "✅ SUCCESS -> Identified and parsed {} entries in {:?}",
                    node_count,
                    start.elapsed()
                );
            }
