...
async fn main() {
    ...
    for port in &tor_ports {
        ...
        let Ok(proxy) = Proxy::all(&proxy_url) else { continue };
            ...
            match client.get(*url).send().await {
                Ok(resp) => {
                    ...
                    if let Ok(body) = resp.text().await {
                        ...
                        println!("  📁 DIRECTORIES:");
                        for d in &dirs {
                            println!("    📂 {}", urlencoding::decode(d).unwrap_or_else(|_| d.clone().into()));
                        }
                        println!("\n  📄 FILES:");
                        for f in &files {
                            println!("    📄 {}", urlencoding::decode(f).unwrap_or_else(|_| f.clone().into()));
                        }
                        
                        // Crawl subdirectories
                        println!("\n  🔍 CRAWLING SUBDIRECTORIES...\n");
                        let base = if final_url.ends_with('/') { final_url.clone() } else { format!("{}/", final_url) };
                        
                        for dir in &dirs {
                            let sub_url = format!("{}{}", base, dir);
                            let decoded_dir = urlencoding::decode(dir).unwrap_or_else(|_| dir.clone().into());
                            println!("  ┌─ 📂 {}", decoded_dir);
                            ...
                            match client.get(&sub_url).send().await {
                                Ok(sub_resp) => {
                                    if let Ok(sub_body) = sub_resp.text().await {
                                        ...
                                        for sf in &sub_files {
                                            println!("  │  📄 {}", urlencoding::decode(sf).unwrap_or_else(|_| sf.clone().into()));
                                        }
                                        for sd in &sub_dirs {
                                            println!("  │  📂 {}", urlencoding::decode(sd).unwrap_or_else(|_| sd.clone().into()));
                                        }
                                        println!("  │  ({} files, {} subdirs)", sub_files.len(), sub_dirs.len());
                                    }
                                }
                                Err(e) => println!("  │  [✗] {}", e),
                            }
...
fn parse_autoindex(body: &str) -> (Vec<String>, Vec<String>) {
    ...
    for cap in link_re.captures_iter(body) {
        ...
        if href == "../" || href == "./" || href.starts_with("?") || href.starts_with("#") { continue; }
        if href.starts_with("http") { continue; }
        if href.starts_with("/fancy/") || href.starts_with("/icons/") { continue; }
