...
async fn main() {
    ...
    let _target_url = "http://fsguestuctexqqaoxuahuydfa6ovxuhtng66pgyr5gqcrsi7qgchpkad.onion/?path=/&token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"; // We need a real token, but we can just fetch the iframe without token first, maybe it redirects, or we fetch the root domain and parse the iframe src.
                        ...
                        match client.get(src).send().await {
                            Ok(resp2) => {
                                if let Ok(text2) = resp2.text().await {
                                    ...
                                    let script_sel =
                                        scraper::Selector::parse("script#__NEXT_DATA__").unwrap();
                                    if let Some(script) = doc2.select(&script_sel).next() {
                                        std::fs::write("/tmp/df_next.json", script.inner_html())
                                            .unwrap();
