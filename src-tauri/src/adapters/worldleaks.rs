...
impl super::CrawlerAdapter for WorldLeaksAdapter {
    async fn can_handle(&self, fingerprint: &super::SiteFingerprint) -> bool {
        fingerprint.body.contains("worldleaks") || fingerprint.body.contains("<app-root></app-root>")
    }
    ...
    async fn crawl(
        ...
        loop {
                    ...
                    workers.spawn(async move {
                        ...
                        for i in 0..items_to_find {
                            ...
                            if f.mark_visited(&dummy_url) {
                                ...
                                let _ = tx_clone.send(dummy_url.clone());
                                
                                let entry = super::FileEntry {
                                    path: format!("/Target Server/{}", dummy_url.replace("http://", "")),
            ...
            if let Some(_res) = workers.join_next().await {
                ...
                // No active tasks and queue is empty
